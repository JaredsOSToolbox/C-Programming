...
void print_contents_side_by_side(FILE* stream, struct file_* a,
                                 struct file_* b) {
    ...
    for (int i = 0; i < longest_side_; ++i) {
        ...
        sprintf(nonvoid_line, "%-75s",
                (i < shortest_side_) ? a->contents[i] : "");
...
void print_contents_side_by_side_mod(FILE* stream, struct file_* a,
                                     struct file_* b) {
    int longest_side_ = (a->length < b->length) ? b->length : a->length;
    int shortest_side_ = (a->length < b->length) ? a->length : b->length;
    ...
    for (int i = 0; i < longest_side_ + begin_shortest_index; ++i) {
        char leftside[BUFSIZ], rightside[BUFSIZ], nonvoid_line[BUFSIZ];
        sprintf(nonvoid_line, "%-75s",
                (i > begin_shortest_index) ? a->contents[j++] : padding);
