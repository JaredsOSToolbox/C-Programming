...
// EXPERIMENTAL CODE FROM DELTA, THE DIFF CLONE
// NOT ENTIRELY WORKING AND NOT BEAUTIFUL

struct file_* file_constructor(const char* file_path, bool standard_in){
...
void read_contents(struct file_* f){
  ...
  FILE* file_in = (f->is_stdin) ? stdin : fopen(f->file_path, "r");
